...
describe('Page d\'une étude - Bouton Manifest', () => {
  ...
  it('Vérifier les informations affichées - Modal', () => {
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Sequencing Data Supplement"] td').eq(1).contains(/^2$/).should('exist');
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Sequencing Data Supplement"] td').eq(2).contains(/^2$/).should('exist');
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Sequencing Data Supplement"] td').eq(3).contains(/^319.69 MB$/).should('exist');
