...
describe('Page d\'une étude (Dataset) - Bouton Manifest', () => {
  ...
  it('Vérifier les informations affichées - Modal [CQDG-1477]', () => {
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Aligned Reads"] td').eq(1).contains(/^1$/).should('exist');
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Aligned Reads"] td').eq(2).contains(/^1$/).should('exist');
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Aligned Reads"] td').eq(3).contains(/^1 B$/).should('exist');
