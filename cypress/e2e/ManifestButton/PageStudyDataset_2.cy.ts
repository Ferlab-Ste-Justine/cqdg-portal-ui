...
describe('Page d\'une étude (Dataset) - Bouton Manifest', () => {
  ...
  it('Vérifier les informations affichées - Modal', () => {
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Germline Structural Variant"] td').eq(3).contains(/^3 B$/).should('exist');
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Unaligned Reads"] td').eq(3).contains(/^4 B$/).should('exist');
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Metrics"] td').eq(3).contains(/^3 B$/).should('exist');
