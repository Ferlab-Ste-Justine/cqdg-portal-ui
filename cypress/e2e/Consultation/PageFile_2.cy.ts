...
describe('Page d\'un fichier - Vérifier les informations affichées', () => {
  ...
  it('Panneau Experimental Procedure', () => {
    ...
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(2).contains('151').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-label"]').eq(3).contains('Source').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(3).contains('Transcriptomic Single Cell').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-label"]').eq(4).contains('Platform').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(4).contains('Illumina HiSeq 2000 PE100').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-label"]').eq(5).contains('Library Selection').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(5).contains('Reduced Representation').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-label"]').eq(6).contains('Capture Kit').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(6).contains('-').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-label"]').eq(7).contains('Targeted Loci').should('exist');
    cy.get('[id="experimental_procedure"] [class="ant-descriptions-item-content"]').eq(7).contains('targetedLoci1 - TDEE').should('exist');
