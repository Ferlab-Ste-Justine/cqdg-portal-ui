...
describe('Page Data Exploration (Data Files) - Valider les liens disponibles', () => {
  it('Lien File du tableau', () => {
    cy.get('tr[data-row-key="FI00093734"] [class*="ant-table-cell"]').eq(3).find('[href]').clickAndWait({force: true});
    cy.get('[id="file-entity-page"]').should('exist');
    cy.get('[class*="EntityTitle"]').contains('FI0009373');
  });

  it('Lien Study du tableau', () => {
    cy.get('tr[data-row-key="FI00093734"] [class*="ant-table-cell"]').eq(4).find('[href]').clickAndWait({force: true});
  ...
  it('Lien Participants du tableau', () => {
    cy.get('tr[data-row-key="FI00093734"] [class*="ant-table-cell"]').eq(10).find('[href]').clickAndWait({force: true});
  ...
  it('Lien Biospecimens du tableau', () => {
    cy.get('tr[data-row-key="FI00093734"] [class*="ant-table-cell"]').eq(11).find('[href]').clickAndWait({force: true});
