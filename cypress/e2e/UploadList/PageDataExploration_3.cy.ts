...
describe('Page Data Exploration (Data Files) - Téléverser une liste d\'identifiants', () => {
  const setupTest = () => {
    ...
    cy.get('[class*="UploadModal"] textarea').type('FH0004562,unknown');
  };
  ...
  it('Valider les fonctionnalités de la modal - Bouton Supprimer', () => {
    setupTest();
    cy.get('[class*="UploadModal"] textarea').contains('FH0004562').should('exist');
    cy.get('[class*="UploadModal"] button[class*="ant-btn-text"]').clickAndWait({force: true});

    cy.get('[class*="UploadModal"] textarea').contains('FH0004562').should('not.exist');
  ...
  it('Vérifier les informations affichées - Section Résumé (onglet Reconnus)', () => {
    ...
    cy.get('[id*="panel-matched"] [data-row-key="FH0004562:0"] td').eq(0).contains('FH0004562').should('exist');
    cy.get('[id*="panel-matched"] [data-row-key="FH0004562:0"] td').eq(1).contains('FH0004562').should('exist');
    cy.get('[id*="panel-matched"] [data-row-key="FH0004562:0"] td').eq(2).contains('T-DEE').should('exist');
  });
