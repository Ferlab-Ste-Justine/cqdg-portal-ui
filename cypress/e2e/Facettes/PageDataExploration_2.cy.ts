...
describe('Page Data Exploration (Biospecimens) - Filtrer avec les facettes', () => {
  ...
  it('Search by sample ID - SR0000084', () => {
    setupTest();
    ...
    cy.typeAndIntercept('[data-cy="SearchAutocomplete_Select"]', 'SR0000084', 'POST', '*/grapgql', 1);
    cy.get('[data-cy="Search_Dropdown"] [class*="ant-select-item"]').contains('SR0000084').should('exist');
    cy.get('[data-cy="Search_Dropdown"] [class*="ant-select-item"]').eq(0).click({force: true});

    cy.get('[data-cy="Tag_SR0000084"]').should('exist');
    ...
    cy.get('[data-cy="Tag_SR0000084"]').should('not.exist');
  });

  it('Search by external sample ID - S15906', () => {
    setupTest();
    cy.typeAndIntercept('[data-cy="SearchAutocomplete_Select"]', 'S15906', 'POST', '*/grapgql', 1);
    cy.get('[data-cy="Search_Dropdown"] [class*="ant-select-item"]').contains('SR0000084').should('exist');
    cy.get('[data-cy="Search_Dropdown"] [class*="ant-select-item"]').eq(0).click({force: true});
  ...
  it('Cancer Tissue Type - Normal Tissue Sample (NCIT:C162623)', () => {
    setupTest();
    cy.validateFacetFilter('Cancer Tissue Type', 'Normal Tissue Sample (NCIT:C162623)', 'Normal Tissue Sample (NCIT:C162623)', /^(9|10)$/);
  ...
  it('Tumor Status - Normal', () => {
    setupTest();
    cy.validateFacetFilter('Tumor Status', 'Normal', 'Normal', /^(9|10|11)$/);
